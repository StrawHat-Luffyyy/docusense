...
webhookRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (eventType === "organization.created" && created_by) {
          ...
          if (internalOrg) {
            await withRetry(
              ...
              "webhook:organizationMember.upsert",
            );
          }
        }
      }
      ...
      logger.error(
        { err: error, eventType, isDbError: true },
        "[DB UNREACHABLE] Error processing webhook data; returning 500 for Clerk retry",
      );
      return res
        .status(500)
        .json({ error: "Database operation failed. Event will be retried." });
    }
  },
);
